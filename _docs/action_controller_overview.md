---
layout: docs
title: Action Controller 简介
prev_section: form_helpers
next_section: routing
---
...
## 控制器的作用 {#what-does-a-controller-do}
...
I> 路由的处理细节请查阅 [Rails Routing From the Outside In]({{ site.baseurl }}/routing.html)。

## 控制器命名约定 {#controller-name-convention}
...
遵守这一约定便可享用默认的路由生成器（例如 `resources` 等），无需再指定 `:path` 或 `:controller`，URL 和路径的帮助方法也能保持一致性。详情参阅 [Layouts & Rendering Guide]({{ site.baseurl }}/layouts_and_rendering.html)。

I> 控制器的命名习惯和模型不同，模型的名字习惯使用单数形式。

## 方法和动作 {#methods-and-actions}
...
详情参阅 [Layouts & Rendering Guide]({{ site.baseurl }}/layouts_and_rendering.html)。
...
### Hash 和数组参数 {#hash-and-array-parameters}
...
I> “[”和“]”这两个符号不允许出现在 URL 中，所以上面的地址会被编码成 `/clients?ids%5b%5d=1&ids%5b%5d=2&ids%5b%5d=3`。大多数情况下，无需你费心，浏览器会为你代劳编码，接收到这样的请求后，Rails 也会自动解码。如果你要手动向服务器发送这样的请求，就要留点心了。

此时，`params[:ids]` 的值是 `["1", "2", "3"]`。注意，参数的值始终是字符串，Rails 不会尝试转换类型。

I> 默认情况下，基于安全考虑，参数中的 `[]`、`[nil]` 和 `[nil, nil, ...]` 会替换成 `nil`。详情参阅[安全指南]({{ site.baseurl }}/security.html#unsafe-query-generation)。
...
I> 解析 XML 格式参数的功能现已抽出，制成了 gem，名为 `actionpack-xml_parser`。
...
#### 允许使用的标量值 {#permitted-scalar-values}
...
#### 不用健壮参数 {#outside-the-scope-of-strong-parameters}
...
关于会话存储的更多内容请参阅[安全指南]({{ site.baseurl }}/security.html)
...
I> 使用 CookieStore 时，如果修改了密令，之前所有的会话都会失效。

### 获取会话 {#accessing-the-session}

在控制器中，可以使用实例方法 `session` 获取会话。

I> 会话是惰性加载的，如果不在动作中获取，不会自动加载。因此无需禁用会话，不获取即可。
...
## 渲染 XML 和 JSON 数据 {#rendering-xml-and-json-data}
...
### 后置过滤器和环绕过滤器 {#after-filters-and-around-filters}
...
### 过滤器的其他用法 {#other-ways-to-use-filters}
...
## 防止请求伪造 {#request-forgery-protection}
...
所有使用[表单帮助方法]({{ site.baseurl }}/form_helpers.html)生成的表单，都有会添加这个令牌。如果想自己编写表单，或者基于其他原因添加令牌，可以使用 `form_authenticity_token` 方法。

`form_authenticity_token` 会生成一个有效的令牌。在 Rails 没有自动添加令牌的地方（例如 Ajax）可以使用这个方法。

[安全指南]({{ site.baseurl }}/security.html)一文更深入的介绍了请求伪造防范措施，还有一些开发网页程序需要知道的安全隐患。

## `request` 和 `response` 对象 {#the-request-and-response-objects}

在每个控制器中都有两个存取器方法，分别用来获取当前请求循环的请求对象和响应对象。`request` 方法的返回值是 `AbstractRequest` 对象的实例；`response` 方法的返回值是一个响应对象，表示回送客户端的数据。

### `request` 对象 {#the-request-object}
...
#### `path_parameters`，`query_parameters` 和 `request_parameters` {#path-parameters-query-parameters-and-request-parameters}
...
### `response` 对象 {#the-response-object}
...
#### 设置自定义报头 {#setting-custom-headers}
...
## HTTP 身份认证 {#http-authentications}
...
### HTTP 基本身份认证 {#http-basic-authentication}
...
### HTTP 摘要身份认证 {#http-digest-authentication}
...
## 数据流和文件下载 {#streaming-and-file-downloads}
...
W> 要谨慎处理用户提交数据（参数，cookies 等）中的文件路径，有安全隐患，你可能并不想让别人下载这个文件。

T> 不建议通过 Rails 以数据流的方式发送静态文件，你可以把静态文件放在服务器的公共文件夹中，使用 Apache 或其他服务器下载效率更高，因为不用经由整个 Rails 处理。

### 使用 REST 的方式下载文件 {#restful-downloads}
...
I> 设置文件不会在每次请求中都重新加载，所以为了让改动生效，需要重启服务器。
...
#### 使用数据流时的注意事项 {#streaming-considerations}
...
### 默认的 500 和 404 模板 {#the-default-500-and-404-templates}
...
I> 某些异常只能在 `ApplicationController` 中捕获，因为在异常抛出前控制器还没初始化，动作也没执行。详情参见 [Pratik Naik 的文章](http://m.onkey.org/2008/7/20/rescue-from-dispatching)。

## 强制使用 HTTPS 协议 {#force-https-protocol}
