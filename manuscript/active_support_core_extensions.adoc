...
WARNING: 注意，这里并没有提到数字。特别说明，`0` 和 `0.0` *不是*空值。
...
异常消息也可静默，使用 `suppress` 方法即可。`suppress` 方法可接受任意个的异常类。如果执行代码块的过程中抛出异常，而且异常属于（`kind_of?`）参数指定的类，`suppress` 方法会静默该异常类的消息，否则抛出异常：
...
Active Support 提供了 `attr_internal_reader`、`attr_internal_writer` 和 `attr_internal_accessor` 三个方法，其行为与 Ruby 内置的 `attr_*` 方法类似，但使用其他方式命名实例变量，减少重名的几率。
...
为了方法，也会生成实例方法，这些实例方法只是类属性的代理。因此，实例可以修改类属性，但是不能覆盖——这与 `class_attribute` 不同（参见上文）。例如：
...
把 `:instance_reader` 设为 `false`，不生成实例读值方法，把 `:instance_writer` 设为 `false`，不生成实例读值方法，把 `:instance_accessor` 设为 `false`，实例读值和设置方法都不生成。此时，这三个选项的值都必须是 `false`，而不能是假值。
...
把数据插入 HTML 模板要格外小心。例如，不能原封不动地把 `@review.title` 内插到 HTML 页面中。加入标题是“Flanagan & Matz rules!”，得到的输出格式就不对，因为 & 会转义成“+&amp;+”。更糟的是，如果应用编写不当，这可能留下严重的安全漏洞，因为用户可以注入恶意的 HTML，设定精心编造的标题。关于这个问题的详情，请阅读 <<security#cross-site-scripting-xss>>对跨站脚本的说明。
...
`truncate_words` 方法在指定个单词除截断接收者，返回一个副本：
...
从技术层面来说，这个方法寻找整个字符串中的最小缩进量，然后把删除那么多的前导空白。
...
`deconstantize` 方法返回限定常量引用表达式最右边那一部分，留下常量的容器：
...
使用 `from_now`、`ago` 等精确计算时期，以及增减 `Time` 对象时使用 `Time#advance`。例如：
...
Rails 大量使用这种语法糖，以此避免编写大量位置参数，模仿具名参数。Rails 经常在最后一个散列选项上使用这种惯用法。
...
然而，如果数组中的元素可以响应 `id` 方法，可以传入参数 `:db`。处理 Active Record 对象集合时经常如此。返回的字符串如下：
...
默认情况下，根元素的名称是第一个元素的类名的复数形式经过 `underscore` 和 `dasherize` 处理后得到的值——前提是余下的元素属于哪个类型（使用 `is_a?` 检查），而且不是散列。在上例中，根元素是“contributors”。
...
为此，这个方法迭代各个键值对，根据值构建节点。加入键值对是 `key, value`：
...
WARNING: `reverse_merge!` 方法会就地修改调用发，这可能不是个好主意。
...
此外，可以使用 `deep_symbolize_keys` 和 `deep_symbolize_keys!` 把指定散列及其中嵌套的散列的键全都转换成富豪。例如：
...
这个方法做计算时，先增加年，然后是月和周，最后是日。这个顺序是重要的，想一个月的末尾流动。假如我们在 2010 年 2 月的最后一天，我们想向前移动一个月和一天。
...
`seconds_since_midnight` 方法返回自午夜的秒数：
