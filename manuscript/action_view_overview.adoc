...
`date_select` 方法返回一组年、月、日的选择标签，用于设置 `date` 类型的属性的值。
...
`datetime_select` 方法返回一组年、月、日、时、分的选择标签，用于设置 `datetime` 类型的属性的值。
...
`select_date` 方法返回一组年、月、日的选择标签，并通过 `Date` 对象来设置默认值。
...
`select_datetime` 方法返回一组年、月、日、时、分的选择标签，并通过 `Datetime` 对象来设置默认值。
...
`select_day` 方法返回一组当月全部天数的选择标签，如 1 到 31，并把当日设置为默认值。
...
`select_hour` 方法返回一组一天中 24 小时的选择标签，即 0 到 23，并把当前小时设置为默认值。
...
`select_minute` 方法返回一组一小时中 60 分钟的选择标签，即 0 到 59，并把当前分钟设置为默认值。
...
`select_month` 方法返回一组一年中 12 个月的选择标签，并把当月设置为默认值。
...
`select_second` 方法返回一组一分钟中 60 秒的选择标签，即 0 到 59，并把当前秒设置为默认值。
...
`select_time` 方法返回一组时、分的选择标签，并通过 `Time` 对象来设置默认值。
...
`select_year` 方法返回一组当年和前后各五年的选择标签，并把当年设置为默认值。可以通过 `:start_year` 和 `:end_year` 选项自定义年份范围。
...
`time_ago_in_words` 方法和 `distance_of_time_in_words` 方法类似，区别在于 `time_ago_in_words` 方法计算的是给定时间到 `Time.now` 给出的当前时间的时间间隔。
...
`time_select` 方返回一组时、分、秒的选择标签（其中秒可选），用于设置 `time` 类型的属性的值。选择的结果作为多个参数赋值给 Active Record 对象。
