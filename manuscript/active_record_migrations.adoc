...
上面的代码把 `products` 数据表的 `part_number` 字段修改为 `:text` 字段。请注意 `change_column` 命令是不可逆的。
...
注意：也可以把上面的 `change_column_default` 迁移写成 `change_column_default :products, :approved, false`，但这种写法是不可逆的。
...
NOTE: Active Record 只支持单字段外键，要使用复合外键需要 `execute` 和 `structure.sql`。更多介绍请参阅 <<active_record_migrationsl#schema-dumping-and-you>>。
