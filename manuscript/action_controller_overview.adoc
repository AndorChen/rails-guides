...
`ApplicationController` 继承自 `ActionController::Base`。后者定义了许多有用的方法。本文会介绍部分方法，如果想知道定义了哪些方法，可查阅 http://api.rubyonrails.org/classes/ActionController.html[API 文档]或源码。
...
`request` 对象中有很多客户端请求的有用信息。可用方法的完整列表参阅 http://api.rubyonrails.org/classes/ActionDispatch/Request.html[Rails API 文档]和 http://www.rubydoc.info/github/rack/rack/Rack/Request[Rack 文档]。下面说明部分属性：
...
`response` 对象通常不直接使用。`response` 对象在动作的执行过程中构建，把渲染的数据回送给用户。不过有时可能需要直接访问响应，比如在后置过滤器中。`response` 对象上的方法有些可以用于赋值。若想了解全部可用方法，参阅 http://api.rubyonrails.org/classes/ActionDispatch/Response.html[Rails API 文档]和 http://www.rubydoc.info/github/rack/rack/Rack/Response[Rack 文档]。
