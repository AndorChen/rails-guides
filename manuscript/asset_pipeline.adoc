...
为 Rails 提供标准 JavaScript 库的 `jquery-rails` gem 就是很好的例子。这个 gem 中包含了继承自 `Rails::Engine` 类的引擎类，这样 Rails 就知道这个 gem 中可能包含静态资源文件，于是会把其中的 `app/assets`、`lib/assets` 和 `vendor/assets` 文件夹添加到 Sprockets 搜索路径中。

[[making-your-library-or-gem-a-pre-processor]]
=== 使用代码库或 gem 作为预处理器

由于 Sprockets 使用 link:$$https://github.com/rtomayko/tilt$$[Tilt] 作为不同模板引擎的通用接口，一个 gem 只要实现了 Tilt 的模板引擎协议就可以用作预处理器。通常的做法是，继承 `Tilt::Template` 类并重新实现 `prepare` 方法（用于初始化模板）和 `evaluate` 方法（返回处理后的内容）。原始内容储存在 `data` 中。更多介绍请参阅 link:$$https://github.com/rtomayko/tilt/blob/master/lib/tilt/template.rb$$[Tilt::Template 类] 的源代码。
...
从 Rails 3.0 或 Rails 2.x 升级时有一些问题需要解决。首先，要把 `public/` 文件夹中的文件移动到新位置。关于不同类型文件储存位置的介绍，请参阅前文中<<asset_pipeline#asset-organization,静态资源文件的组织方式>>一节。
