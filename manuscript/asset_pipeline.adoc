...
如果文件请求包含哈希值，例如 `public/assets/rails-af27b6a414e6da00003503148be9b409.png`，处理的方式也是一样的。关于如何生成哈希值的介绍，请参阅 <<in-production>>。
...
由于 Sprockets 使用 link:https://github.com/rtomayko/tilt[Tilt] 作为不同模板引擎的通用接口，一个 gem 只要实现了 Tilt 的模板引擎协议就可以用作预处理器。通常的做法是，继承 `Tilt::Template` 类并重新实现 `prepare` 方法（用于初始化模板）和 `evaluate` 方法（返回处理后的内容）。原始内容储存在 `data` 中。更多介绍请参阅 link:https://github.com/rtomayko/tilt/blob/master/lib/tilt/template.rb[`Tilt::Template` 类的源码]。
