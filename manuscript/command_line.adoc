...
[NOTE]
====
`type` 参数可用的全部字段类型参见 `SchemaStatements` 模块中 http://api.rubyonrails.org/classes/ActiveRecord/ConnectionAdapters/SchemaStatements.html#method-i-add_column[`add_column` 方法的 API 文档]。`index` 参数为相应的列生成索引。
====
