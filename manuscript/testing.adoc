...
一个测试中可以有一个或多个断言，对断言的数量没有限制。只有所有断言都成功，测试才能通过。
...
我们大致了解了几个可用的断言。断言是测试的核心所在，是真正执行检查，确保功能复合预期的执行者。
...
Minitest 这个测试框架时模块化的，因此还可以自己创建断言。事实上，Rails 就这么做了。Rails 提供了一些专门的断言，能简化测试。
...
固件代指示例数据，在运行测试之前，使用预先定义好的数据填充测试数据库。固件与所用的数据库没有关系，是使用 YAML 格式编写。一个模型有一个固件文件。
...
[TIP]
====
为了从数据库中删除现有数据，Rails 会尝试禁用引用完整性触发器（如外键和约束检查）。运行测试时，如果见到烦人的权限错误，确保数据库用于有权在测试环境中禁用这些触发器。（对 PostgreSQL 来说，只有超级用户能禁用全部触发器。关于 PostgreSQL 权限的详细说明参阅link:http://blog.endpoint.com/2012/10/postgres-system-triggers-error.html[这篇文章]。）
====
...
集成测试运行程序的说明参阅 ActionDispatch::Integration::Runner[`ActionDispatch::Integration::Runner` 模块的文档]。

执行请求的方法参见 http://api.rubyonrails.org/classes/ActionDispatch/Integration/RequestHelpers.html[`ActionDispatch::Integration::RequestHelpers` 模块模块的文档]。
...
上述命令回味 `Articles` 资源生成控制器和测试。你可以看一下 `test/controllers` 目录中的 `articles_controller_test.rb` 文件。
...
如果想测试 Ajax 请求，要在 `get`、`post`、`patch`、`put` 和 `delete` 方法中设定 `xhr: true` 选项。例如：
...
你可能还记得，早功能测试中可用的三个散列中有一个是 `flash`。
...
测试请求的响应中是否出现关键的 HTML 元素和相应的内容是测试应用视图的一种常见方式。与路由测试一样，视图测试放在 `test/controllers/` 目录中，或者与直接写在控制器测试中。`assert_select` 方法用于查询响应中的 HTML 元素，其句法简单而强大。
