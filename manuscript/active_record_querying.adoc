...
如果上面的代码没有使用 `where` 方法，就会生成正常的一组两条查询语句。
...
通过在 `where` 方法中使用字符串 SQL 片段并使用 `references` 方法这种方式，即便一条评论都没有，所有文章仍然会被加载。而在使用 `joins` 方法（`INNER JOIN`）时，必须匹配关联条件，否则一条记录都不会返回。
